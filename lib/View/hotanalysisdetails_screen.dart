...
class HotAnalysisDetailsPage extends StatefulWidget {
  final DateTime startDate;
  final DateTime endDate;
  final List<Map<String, String>> selectedTags;

  const HotAnalysisDetailsPage({
    super.key,
  ...
  @override
  State<HotAnalysisDetailsPage> createState() => _HotAnalysisDetailsPageState();
}

class _HotAnalysisDetailsPageState extends State<HotAnalysisDetailsPage> {
  bool _loading = true;
  ...
  ConsumptionStats _getHot(AnalyticsViewModel vm) {
  ...
  List<ChartDataBar> _buildBarsHotOnly(AnalyticsViewModel vm) {
    final res = vm.analyticsResponse;
    final List<ChartDataBar> data = [];
    if (res == null || res.dates.isEmpty || res.consumptions.isEmpty) return data;
    ...
    for (int i = 0; i < res.dates.length; i++) {
      ...
      data.add(ChartDataBar(res.dates[i], 0.0, hotValue));
    }
    return data;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final hot = _getHot(analyticsVM);

    final bars = _buildBarsHotOnly(analyticsVM);
      ...
      body: _loading
          ...
          : ListView(
              ...
              children: [
                ...
                Text(
                  "${hot.total.toStringAsFixed(0)}Litres",
                  style: TextStyle(
                    color: fg,
                    fontSize: 26,
                    fontWeight: FontWeight.w800,
