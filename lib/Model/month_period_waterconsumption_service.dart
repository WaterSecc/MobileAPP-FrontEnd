...
class MonthPeriodConsumptionService {
  final _httpClient = http.Client();
  String urlProd = dotenv.env['URLPRODB2B'] ?? '';
  String periodstats = dotenv.env['PERIODSTATS'] ?? '';

  Future<ConsumptionDataResponse> getMonthPeriodConsumption(
    ...
    if (selectedWaterMeterIds.isNotEmpty) {
      String locationsQuery = selectedWaterMeterIds
          ...
          .join('&');
      url = Uri.parse('$urlProd${periodstats}month?$locationsQuery');
    } else {
      // Default URL without water meter selection
      url = Uri.parse('$urlProd${periodstats}month');
    }
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to fetch water consumption data for the period: month');
    }
  }
}
