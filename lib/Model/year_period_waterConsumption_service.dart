...
class YearPeriodConsumptionService {
  final _httpClient = http.Client();
  String urlProd = dotenv.env['URLPRODB2B'] ?? '';
  String periodstats = dotenv.env['PERIODSTATS'] ?? '';

  Future<ConsumptionDataResponse> getYearPeriodConsumption(
    ...
    if (selectedWaterMeterIds.isNotEmpty) {
      String locationsQuery = selectedWaterMeterIds
          ...
          .join('&');
      url = Uri.parse('$urlProd${periodstats}year?$locationsQuery');
    } else {
      // Default URL without water meter selection
      url = Uri.parse('$urlProd${periodstats}year');
    }
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to fetch water consumption data for the period: year');
    }
  }
}
